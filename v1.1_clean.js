...
// set this a variabel
//TLV: center: [32.03993,34.82497]
var map = L.map('map', {
    center: [52.358301, 4.863396],
    zoom: 13
});
...
const plane_ar = []
for (var i in track.ams22[0].features[0].geometry.coordinates) {
    plane_ar.push({ 'lat': track.ams22[0].features[0].geometry.coordinates[i][1], 'lng': track.ams22[0].features[0].geometry.coordinates[i][0], 'ele': track.ams22[0].features[0].geometry.coordinates[i][2] })
...
function planeDraw(airlab) {
    ...
    airlab.response.forEach((plane, i) => {
        ...
        var planeMarker = L.rotatedMarker([lat, lon], {
            icon: planeicon,
            rotationAngle: plane.dir
        }).bindPopup(popup)
